...
class dataProcessing:
    ...
    def dataPreparation(self): #to fetch lat long, we need to have the details of both plant side and customer side
        ...
        Facility_Master = pd.merge(
             node_facility_master[['facility', 'facility_code', 'facility_desc', 'facility_processing', 'facility_sub_type', 'validated_by', 'i2_code', 'address_id_id']],
             node_address[['id', 'lat', 'lng']], how='left', left_on='address_id_id', right_on='id'
        ).drop(columns=['address_id_id', 'id']).merge(
             facility_master[['facility_code', 'depot_code', 'depot_desc']], how='left', on='facility_code'
        ...
        Cust_Master = pd.merge( node_customer_master[['customer_code', 'is_active', 'validated_by', 'address_id_id']],  node_address[[
                 ...
                 customer_master[['city_code', 'city_descr', 'taluka_code', 'taluka_descr', 'district_code', 'district_description', 'depot_code', 'depot_description', 'region_code', 'region_description', 'state_code', 'state_description', 'zone_code', 'zone_descr', 'country', 'active']], 
    ...
    def importing_monitored_trip_data(self, MonthRun, MonitoredTripGPSData_Input_mypath): #want to use Output (return of last function) here
        ...
        # MonitoredTrip_Axestrack =  importing_file(MonitoredTripGPSData_Input_mypath)
...
class waypoint:
    ...
    def __init__(self, cur, dataj, unique_tripID_monitored, unique_tripID_GPS_data, Waypoint_WCustFclty_WLatLng, geolocator): #initialization function
         cursor = cur.cursor()
         dataj = dataj
         unique_tripID_monitored = unique_tripID_monitored
         unique_tripID_GPS_data = unique_tripID_GPS_data
         Waypoint_WCustFclty_WLatLng = Waypoint_WCustFclty_WLatLng
         geolocator = geolocator
        
        
    def fetch_GPS_data(trip_list):
        conn = connection_to_database(database_name_dev,host_name_dev,port_dev,user_name_dev,user_password_dev)
        cur = conn.cursor()
        #Fetch GPS ping data from DB for all the unique TIDs
        l = trip_list
    ...
    def common_tripID_func(self): #finding out the common trip IDs
        common_tripID =  unique_tripID_monitored.merge( unique_tripID_GPS_data, on = 'Trip ID', how= 'inner').merge(dataj, on = 'Trip ID', how= 'inner')
    ...
    def unique_plant_taluka_combination_func(self,common_tripID): #grouping the common trip ID by group & taluka to form its unique combination
        common_tripID_whole_Data =  dataj.loc[ dataj['Trip ID'].isin(set(common_tripID['Trip ID']))].reset_index(drop=True)
    ...
    def trip_list_func(common_tripID_whole_Data, unique_plant_taluka_combination, ctr):
    ...
    def check2_GPS_trip_location_details(GPS_trip_location_details, trip_list, dataj,unique_plant_taluka_combination, ctr):
    ...
    def Axestrack_MergeGPS_func(dataj_, GPS_trip_location_details):
    ...
    def count_trips_latlng_func(GPS_trip_location_details):
    ...
    def check3_unique_trips_count(total, unique_trips_count, Axestrack_MergeGPS):
    ...
    def check3_trip_wayroute_details(total,unique_trips_count,Axestrack_MergeGPS):
        df = Axestrack_MergeGPS[Axestrack_MergeGPS['Trip ID'] == unique_trips_count[total]].reset_index(drop = True)
    ...
    def check4_trip_elimination(df2, trip_wayroute_details):        
    ...
    def hvrsne_distance_calc(from_lat_lng, to_lat_lng):
        return pd.Series(haversine_vector(from_lat_lng, to_lat_lng, Unit.KILOMETERS))
            
    def check5_trip_wayroute_details_dest_null(trip_wayroute_details):
    ...
    def check6_trip_elimination(df2, trip_wayroute_details):
    ...
    def check7_trip_elimination(df2, trip_wayroute_details):
    ...
    def check8_trip_elimination(df2, trip_wayroute_details,v1):
    ...
    def percentile_distance_origin_calc(trip_wayroute_details, prcnt): #function to calculate percentile distance from origin
        return np.percentile(trip_wayroute_details['HvrsineDistOrgn_latlng'], prcnt)
                        
    def percentile_distance_dest_calc(trip_wayroute_details, prcnt):
        return np.percentile(trip_wayroute_details['HvrsineDistDest_latlng'], prcnt)
    
    def trips_outside_range_func(trip_wayroute_details, Orgn_DistPrcnt85, Dest_DistPrcnt5):
    ...
    def OnwardJourneyGPSData_func(trip_wayroute_details, present_truck_gps_attributes_raw, onward_trip_dest_LtLngGPS):
    ...
    def gps_pings_func(OnwardJourneyGPSData_1):
    ...
    def specifc_trip_func(gps_pings, specific_trip_):
    ...
    def check9_reverse_geolocator(master_df, specific_trip):
    ...
    def way_OnwardJourneyGPSData_func(OnwardJourneyGPSData):
    ...
    def Waypoint_Latlng_func(OnwardJourneyGPSData):
    ...
    def check10_trip_elimination(df2, Waypoint_LatLng):
    ...
    def assigned_Waypoint_LatLng(df2, Waypoint_LatLng):
    ...
    def find_LeadHvrsineMAPE_func(MD_Trip):
        MD_Trip['Lead_HvrsineMAPE'] = abs(MD_Trip['Lead'] - MD_Trip['HvrsineAggrtd'])/MD_Trip['Lead']
        return MD_Trip
    
    def unique_plant_city_combination(MD_Trip):
    ...
    def check11_selecting_routes(df, TripID_Waypoint_LatLng, UnqPlntCityCd_WypntMD, i2):
    ...
    def check12_waypoint_LatLng_All_Routes_selected(TripID_Waypoint_LatLng, WWaypoint_MD_Trip_WminMAPE):
    ...
    def analysis(unique_plant_taluka_combination):
        ...
        unique_plant_taluka_combination = unique_plant_taluka_combination.head()
        for ctr in range(len(unique_plant_taluka_combination)):
            ...
            trip_list = trip_list_func(common_tripID_whole_Data, unique_plant_taluka_combination, ctr)
                        
            if len(trip_list)>0: #check1 
                GPS_trip_location_details = fetch_GPS_data(trip_list)
            else:
                GPS_trip_location_details = pd.DataFrame()
            
            if len(GPS_trip_location_details) > 0: #check2
                dataj_ = check2_GPS_trip_location_details(GPS_trip_location_details, trip_list, dataj, unique_plant_taluka_combination, ctr)
                Axestrack_MergeGPS = Axestrack_MergeGPS_func(dataj_, GPS_trip_location_details)
                Axestrack_MergeGPS.to_csv('Axestrack_MergeGPS.csv')
                GPS_trip_location_details.to_csv('GPS_trip_location_details.csv')
                                
                count_trips_wrt_latlng = count_trips_latlng_func(GPS_trip_location_details)
                ...
                Axestrack_MergeGPS.to_csv('Axestrack_MergeGPS_after_merging_with_latlng_trips.csv')
                unique_trips_count = Axestrack_MergeGPS['Trip ID'].unique()
                total = 0
                # checkpoint3
                while (total < len(unique_trips_count)) & (len(unique_trips_count)>0):
                    df2 = check3_unique_trips_count(total, unique_trips_count, Axestrack_MergeGPS)
                    trip_wayroute_details = check3_trip_wayroute_details(total, unique_trips_count, Axestrack_MergeGPS)
                    # trip ID elimination
                    if (len(trip_wayroute_details)/df2.loc[0,'TripID_rcrdCnt'] < .2) | (len(trip_wayroute_details) < 2): #checkpoint4
                        df2 =  check4_trip_elimination(df2, trip_wayroute_details)
                        ...
                        trip_wayroute_details.loc[:, 'HvrsineDistOrgn_latlng'] = hvrsne_distance_calc(from_lat_lng, to_lat_lng)
                        
                        if len(trip_wayroute_details[trip_wayroute_details['Dest_lat'].isnull()]) > 0: #checkpoint5 if the destnation geog details are null
                            trip_wayroute_details =  check5_trip_wayroute_details_dest_null(trip_wayroute_details)
                        ...
                        to_lat_lng = trip_wayroute_details[['latitude', 'longitude']].to_numpy()
                        trip_wayroute_details.loc[:, 'HvrsineDistDest_latlng'] = hvrsne_distance_calc(from_lat_lng, to_lat_lng)
                        ...
                        if (trip_wayroute_details.loc[0,'HvrsineDistOrgn_latlng'] - trip_wayroute_details.loc[len(trip_wayroute_details)-1,'HvrsineDistOrgn_latlng'] > max(trip_wayroute_details.loc[:,'HvrsineDistOrgn_latlng'])*.5): #checkpoint 6
                            df2 = check6_trip_elimination(df2, trip_wayroute_details)
                            MD_Trip = pd.concat([MD_Trip, df2], ignore_index= True)
                            ...
                            if (min(trip_wayroute_details.loc[:,'HvrsineDistDest_latlng'])/max(trip_wayroute_details.loc[:,'HvrsineDistDest_latlng']) > .1): #checkpoint 7
                                df2 = check7_trip_elimination(df2, trip_wayroute_details)
                                MD_Trip = pd.concat([MD_Trip, df2], ignore_index= True)
                                
                            else:
                                if (trip_wayroute_details.loc[0,'HvrsineDistOrgn_latlng']/v1.iloc[0] > .1): #checkpoint 8
                                    df2=  check8_trip_elimination(df2, trip_wayroute_details)
                                    MD_Trip = pd.concat([MD_Trip, df2], ignore_index= True)
                                
                                else:
                                    Orgn_DistPrcnt85 = min(trip_wayroute_details.loc[0,'HvrsineDistDest_latlng'], percentile_distance_origin_calc(trip_wayroute_details,85))
                                    Dest_DistPrcnt5 = max( percentile_distance_dest_calc(trip_wayroute_details,5),.01)
                                    
                                    onward_trip_dest_LtLngGPS =  trips_outside_range_func(trip_wayroute_details, Orgn_DistPrcnt85, Dest_DistPrcnt5)
                                    
                                    if (len(onward_trip_dest_LtLngGPS) == 0) : #checkpoint 9
                                        Dest_DistPrcnt5 =  percentile_distance_dest_calc(trip_wayroute_details,5)
                                        onward_trip_dest_LtLngGPS = trip_wayroute_details[(trip_wayroute_details['HvrsineDistDest_latlng'] < Dest_DistPrcnt5)]
                                    
                                    OnwardJourneyGPSData_1 =  OnwardJourneyGPSData_func(trip_wayroute_details, present_truck_gps_attributes_raw, onward_trip_dest_LtLngGPS)
                                    specific_trip_exist, gps_pings =  gps_pings_func(OnwardJourneyGPSData_1)
                                   
                                    specific_trip =  specific_trip_func(gps_pings, specific_trip_)
                                   
                                    if len(specific_trip)> 0: #check9 getting the lat-long details from the name of the place
                                       specific_trip, master_df =  check9_reverse_geolocator(master_df, specific_trip)
                                    ...
                                    OnwardJourneyGPSData =  way_OnwardJourneyGPSData_func(OnwardJourneyGPSData)
                                    
                                    Waypoint_Latlng =  Waypoint_Latlng_func(OnwardJourneyGPSData)
                                    
                                    from_lat_lng = Waypoint_Latlng[['lag_lat', 'lag_lng']].to_numpy()
                                    to_lat_lng = Waypoint_Latlng[['Lat', 'Long']].to_numpy()

                                    Waypoint_LatLng.loc[:,'HvrsnDist_WayPointLatLng'] =  hvrsne_distance_calc(from_lat_lng, to_lat_lng)
                                    
                                    df2 =  check10_trip_elimination(df2, Waypoint_LatLng) #checkpoint 10 for trip elimination
                                    
                                    Waypoint_LatLng =  assigned_Waypoint_LatLng(df2, Waypoint_LatLng)
            ...
            MD_Trip = find_LeadHvrsineMAPE_func(MD_Trip) #finding HvrsineMAPE by subtracting HvrsineAggrtd from Lead and dividing by Lead
            UnqPlntCityCd_WypntMD = unique_plant_city_combination(MD_Trip)
            for j in range(len(UnqPlntCityCd_WypntMD)): #selecting the best route for a plant-city combination out of the recorded trip IDs
                ...
                if len(df) > 1: #checkpoint 11
                    df =  check11_selecting_routes(df, TripID_Waypoint_LatLng, UnqPlntCityCd_WypntMD)
                    if len(df) > 1:
            ...
            if len(WWaypoint_MD_Trip_WminMAPE) > 0: #checkpoint 12
                Waypoint_LatLng_All_FnlRouteSlctd_All =  check12_waypoint_LatLng_All_Routes_selected(TripID_Waypoint_LatLng, WWaypoint_MD_Trip_WminMAPE)
            
        #MD_Trip_.to_csv(r"C:\Users\nikhil.upadhyay-v\Downloads\SED\output datsets\MD_Trip_jan.csv",index=False)
        MD_Trip_.to_csv('MD_Trip_.csv')
...
if __name__ == '__main__':
    ...
    MonthRun = "Mar23" #generate waypoint records for the month
    ...
    Waypoint_WCustFclty_WLatLng.to_csv('Waypoint_WCustFclty_WLatLng.csv')
    ...
    wp.analysis(unique_plant_taluka_combination)
